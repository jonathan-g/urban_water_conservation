...
\replaced{sharply peaked}{skewed, with a sharp peak} near \Sexpr{filtered_data$msa_data %>% summarize(median(pop)) %>% simplify() %>% signif(1) %>% scales::comma() %>% unname()}%
...
ggsave(file.path(si_output_dir, 'figures', 'fig_S1.pdf'), p2, 'pdf', height=8.0, width = 15.0)
...
<<vwci_years_cat_plots, cache=TRUE, include=TRUE, dependson="load_data", fig.cap="Regression coefficients for VWCI, averaging state and MSA aridity over different intervals.", fig.height=8, fig.width=6, out.width="6.25in", cache.extra=c(loo_model_fits)>>=
...
param_levels <- levels(model_ggs$ggs_2_ml_beta_alpha$Parameter)

extra_levels <- model_ggs %>% keep(str_detect(names(.), '^ggs_[4-5]')) %>%
  map(~levels(.x$Parameter)) %>% simplify() %>% unname() %>% unique() %>% setdiff(param_levels)

dens_levels <- levels(model_ggs$ggs_3_ml_beta_alpha$Parameter) %>% setdiff(param_levels) %>%
...
limits <- model_ggs$ggs_2_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
...
limits <- model_ggs$ggs_6_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
  group_by(Parameter) %>% summarize(lower = quantile(value, 0.025), upper = quantile(value,0.975)) %>%
  ungroup() %>% bind_rows(limits) %>%
  summarize(lower = min(lower), upper = max(upper))
limits <- model_ggs$ggs_7_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
  group_by(Parameter) %>% summarize(lower = quantile(value, 0.025), upper = quantile(value,0.975)) %>%
  ungroup() %>% bind_rows(limits) %>%
  summarize(lower = min(lower), upper = max(upper))
limits <- model_ggs$ggs_8_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
  group_by(Parameter) %>% summarize(lower = quantile(value, 0.025), upper = quantile(value,0.975)) %>%
  ungroup() %>% bind_rows(limits) %>%
  summarize(lower = min(lower), upper = max(upper)) %>%
  mutate_all(funs(./4)) %>%
...
p2 <- generate_cat_plot(model_ggs$ggs_2_ml_beta_alpha, "vwci", " (a) 1985-2014",
                        limits = limits, legend = TRUE) +
...
p6 <- generate_cat_plot(model_ggs$ggs_6_ml_beta_alpha, "vwci", " (b) 1970-2014",
                        limits = limits, legend = FALSE) +
...
p7 <- generate_cat_plot(model_ggs$ggs_7_ml_beta_alpha, "vwci", " (c) 1995-2014",
                        limits = limits, legend = FALSE) +
  theme(axis.title.x = element_blank(),
        plot.title = element_text(size = rel(0.8)),
        axis.text.x = element_text(size = rel(0.7)),
...
p8 <- generate_cat_plot(model_ggs$ggs_8_ml_beta_alpha, "vwci", " (d) 2005-2014",
                        limits = limits, legend = FALSE) +
...
p_cat_years <- egg::ggarrange(p2, p6, p7, p8, ncol = 1, draw = FALSE)
...
<<vwci_vars_cat_plots, cache=TRUE, include=TRUE, dependson="vwci_years_cat_plots", fig.cap="Regression coefficients for VWCI, with different covariates.", fig.height=8, fig.width=6, out.width="6.25in">>=
limits <- model_ggs$ggs_2_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
  group_by(Parameter) %>% summarize(lower = quantile(value, 0.025), upper = quantile(value,0.975)) %>%
  ungroup() %>% summarize(lower = min(lower), upper = max(upper))
limits <- model_ggs$ggs_3_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
  group_by(Parameter) %>% summarize(lower = quantile(value, 0.025), upper = quantile(value,0.975)) %>%
  ungroup() %>% bind_rows(limits) %>%
  summarize(lower = min(lower), upper = max(upper))
limits <- model_ggs$ggs_4_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
  group_by(Parameter) %>% summarize(lower = quantile(value, 0.025), upper = quantile(value,0.975)) %>%
  ungroup() %>% bind_rows(limits) %>%
  summarize(lower = min(lower), upper = max(upper))
limits <- model_ggs$ggs_5_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
  group_by(Parameter) %>% summarize(lower = quantile(value, 0.025), upper = quantile(value,0.975)) %>%
  ungroup() %>% bind_rows(limits) %>%
  summarize(lower = min(lower), upper = max(upper)) %>%
  mutate_all(funs(./4)) %>%
...
p2 <- generate_cat_plot(model_ggs$ggs_2_ml_beta_alpha, "vwci", " (a) baseline",
                        limits = limits, legend = TRUE) +
  theme(legend.position = c(0.99,0.01),
        axis.title.x = element_blank(),
        plot.title = element_text(size = rel(0.8)),
...
p3 <- generate_cat_plot(model_ggs$ggs_3_ml_beta_alpha, "vwci", " (b) substituting population density",
                        limits = limits, legend = FALSE) +
  theme(axis.title.x = element_blank(),
        plot.title = element_text(size = rel(0.8)),
        axis.text.x = element_text(size = rel(0.7)),
...
p4 <- generate_cat_plot(model_ggs$ggs_4_ml_beta_alpha, "vwci", " (c) adding MSA area",
                        limits = limits, legend = FALSE) +
  theme(axis.title.x = element_blank(),
        plot.title = element_text(size = rel(0.8)),
        axis.text.x = element_text(size = rel(0.7)),
...
p5 <- generate_cat_plot(model_ggs$ggs_5_ml_beta_alpha, "vwci", " (d) adding Gini index",
                        limits = limits, legend = FALSE) +
  theme(axis.title.x = element_text(size = rel(0.8)),
        plot.title = element_text(size = rel(0.8)),
        axis.text.x = element_text(size = rel(0.7)),
...
p_cat_vars <- egg::ggarrange(p2, p3, p4, p5, ncol = 1, draw = FALSE)
...
state_tbl <- state_data %>% arrange(state.name) %>%
  mutate(surface.water = round(100 * surface.water, 3),
...
table_counter <- table_counter + 1
@
<<loo_table_setup, cache = TRUE, include = FALSE, warning=FALSE, message=FALSE, dependson='load_data'>>=
...
model_names <- list(
  years = c('ml' = 'hierarchical', 'sl' = 'single-level', 'beta' = 'beta-', '_alpha'='',
            '_'=' ', '- $' = '-',
            '^sfit 2[_ ]+(.*)$' = '\\1: 1985--2014', '^sfit 3[_ ]+(.*)$' = '\\1: 1985--2014 with density,',
            '^sfit 4[_ ]+(.*)$' = '\\1: 1985--2014 with area,', '^sfit 5[_ ]+(.*)$' = '\\1: 1985--2014 with Gini index,',
            '^sfit 6[_ ]+(.*)$' = '\\1: 1970--2014', '^sfit 7[_ ]+(.*)$' = '\\1: 1995--2014',
            '^sfit 8[_ ]+(.*)$' = '\\1: 2005--2014'),
  vars = c('ml' = 'hierarchical', 'sl' = 'single-level', 'beta' = 'beta-', '_alpha'='',
           '_'=' ', '- $' = '-',
           '^sfit 2[_ ]+(.*)$' = '\\1: baseline', '^sfit 3[_ ]+(.*)$' = '\\1: population density,',
           '^sfit 4[_ ]+(.*)$' = '\\1: with area,', '^sfit 5[_ ]+(.*)$' = '\\1: with Gini index,',
           '^sfit 6[_ ]+(.*)$' = '\\1: 1970--2014', '^sfit 7[_ ]+(.*)$' = '\\1: 1995--2014',
           '^sfit 8[_ ]+(.*)$' = '\\1: 2005--2014')
)

model_sets <- list(
  years = str_c('^sfit_', c("2", "6", "7", "8"), collapse = "|"),
  vars = str_c('^sfit_', c("2", "3", "4", "5"), collapse = "|")
)

compare_ic <- function(df, ic, criterion) {
  if (is.character(ic)) {
    ic <- str_to_lower(ic)
    if (ic == 'loo') { ic <- loo::loo }
    else if (ic == "waic") { ic <- loo::waic }
  ...
  df[str_detect(names(df), model_sets[[criterion]])] %>% set_names(
    ...
    lapply(ic) %>% loo::compare(x = .) %>% as.data.frame() %>% rownames_to_column('model') %>%
...
ic_table <- function(df, ic, subset, caption) {
  ...
  tbl <- df %>%
    rename(Model = model, !!ic_name := !!ic_column, !!ic_se_name := !!ic_se_column,
           !!elpd_name := !!elpd_column, !!elpd_se_name := !!elpd_se_column) %>%
    xtable(label=str_c("tab:", str_c(ic,subset,sep=".")) , align=c('r', 'r','r','r','r','r'),
...
@
<<loo_table_years, cache = TRUE, include = TRUE, warning=FALSE, message=FALSE, dependson='loo_table_setup', results="asis">>=
...
loo_caption <- c("Comparing different climatological averaging periods and models: LOO = leave-one-out cross-validation, LOO-IC = LOO information criterion, ELPD = expected log pointwise predictive density, and s.e.\ indicates the standard error of estimates of quantities. Lower values of the information criteria and greater (less negative) values of ELPD indicate superior model performance. Models are labelled by the time-period for averaging aridity and the structure of the statistical mode.", "Model comparison: LOO (climatological interval).")

tbl <- ic_table(loo_ic_years, "loo", "years", loo_caption)
@
<<loo_table_vars, cache = TRUE, include = TRUE, warning=FALSE, message=FALSE, dependson='loo_table_setup', results="asis">>=
loo_var_table_num <- table_counter
table_counter <- table_counter + 1
cat(str_c("\\subsection*{Table S", loo_var_table_num, "}", "\n"))

loo_ic_vars <- compare_ic(lwmf, "loo", "vars")

loo_caption <- c("Comparing different models and sets of covariates with LOO-IC. Models are labelled by the covariates that differ from the baseline case and the baseline and by the structure of the statistical mode.", "Model comparison: LOO (covariates).")

tbl <- ic_table(loo_ic_vars, "loo", "vars", loo_caption)
@
<<waic_table_years, cache = TRUE, include = TRUE, warning=FALSE, message=FALSE, dependson='loo_table_setup', results="asis">>=
...
waic_caption <- c("Model comparison: WAIC = widely applicable information criterion (also known as the Watanabe-Aikake Information Criterion), ELPD = expected log-probability density, and s.e.\ indicates the standard error of estimates of quantities. Lower values of the information criteria and greater (less negative) values of ELPD indicate superior model performance. Models are labelled by the time-period for averaging aridity and the structure of the statistical mode.", "Model comparison: WAIC (climatological interval).")

tbl <- ic_table(waic_ic_years, "waic", "years", waic_caption)
@
<<waic_table_vars, cache = TRUE, include = TRUE, warning=FALSE, message=FALSE, dependson='loo_table_setup', results="asis">>=
waic_var_table_num <- table_counter
table_counter <- table_counter + 1
cat(str_c("\\subsection*{Table S", waic_var_table_num, "}", "\n"))
waic_ic_vars <- compare_ic(lwmf, "waic", "vars")

waic_caption <- c("Model comparison with WAIC. Models are labelled by the covariates that differ from the baseline case and by the structure of the statistical mode.", "Model comparison: WAIC (climatological interval).")

tbl <- ic_table(waic_ic_vars, "waic", "vars", waic_caption)
@
