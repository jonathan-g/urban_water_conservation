...
\deleted{and higher partisan voting index (PVI) }%
\explain{Move discussion of MSA-level PVI versus state-level PVI and difference between
state- and MSA-level out of the abstract, as it requires more subtle discussion.}%
tend to adopt more conservation measures.
Within a state, cities with
\added{more Democratic-leaning voting preferences and }%
\explain{As reviewer \#2 noted, most readers won't be familiar with PVI, so explaining
the results in terms of Democratic vs.\ Republican voting will be clearer and more accessible.}%
large and rapidly growing populations\deleted{ and higher PVI than the rest of the state}
tend to adopt more conservation measures.
Economic factors and climatic differences between cities do not correlate with
the number of measures adopted, but they do \replaced{correspond to}{correlate with}
...
water from distant or difficult sources\replaced{, but}{. However,}
many have also shown increasing
interest in the soft path,\added{ which involves}
...
conservation indices are available, but these \added{studies} either lack comprehensive coverage
...
city's or state's preference for the Democratic\added{ (more progressive)}
or Republican\added{ (more conservative)} candidates,
respectively, relative to the national average.
\added{Previous studies have indicated that a more Democratic-leaning PVI score is associated with higher environmental
...
\citep{svara:city.councils:2003} and \added{that} more than one third of state-legislature
...
voting patterns are \replaced{widely used to gauge}{a useful measure for gauging} regional variations in partisan
leanings.
...
%
Our previous analysis found that the \replaced{PVI}{measure of Democratic-leaning voting preference (PVI)}
...
The importance of \replaced{PVI}{this measure}
was supported by qualitative analysis, which suggested that\added{ the} political
...
\replaced{47 states}{the United States} \citep{hess:vwci:2017},
which\added{ span 47 and} comprise more than half of the 382~MSAs
in the \replaced{United States}{U.S.}
\citep{census:population:2015}\replaced{ and here}{. Here}, we present the first quantitative
...
\replaced{This}{Our goal is to assess differences across MSAs that are correlated with higher or lower levels of water-conservation policy adoption. Our} analysis\added{ also}
considers state-level\deleted{ as well as MSA-level}
variables\added{ because MSA policy adoption can be strongly influenced by state-government actions, such as the California state government's strong policy support for water conservation)}.
We identify which variables correlate most strongly with variations in water conservation policy
regimes\deleted{, how effects at the state level moderate city-level effects,} and whether these
...
%
\begin{figure}[htp]
\includegraphics[width=6.5in,angle=0]{figures/vwci_map-1} \caption[Map of cities with VWCI scores]{Map of cities with VWCI scores.}\label{fig:vwci_map}
\end{figure}
...
{\centering \includegraphics[width=0.8\linewidth]{figures/vwci_histogram-1} 
...
The\deleted{ full} VWCI data set \replaced{provides detailed information about}{reports} both the number and
...
\added{The set of 79 measures for each city makes it possible to construct different variables.
...
The \replaced{number of requirements}{requirement scores} range\deleted{s}
from~0
to~23
with a mean of 5.6
and a median of 4
(Figure~\ref{fig:vwci_histogram}b)
and the \replaced{number of rebates}{rebate scores} range\deleted{s}
from~0
to~15
with a mean of 2.7
and a median of 2
...
\added{Our previous qualitative research
...
considerably (Tables~\ref{tab:top_vwci}, S1)\deleted{, and our previous qualitative research
indicated that there might be a preference for rebates over requirements in more
politically conservative cities
[\textit{Hess et al.}, 2016, \textit{Brown and Hess}, 2017]
Consequently, in addition to the VWCI score we also analyzed the number of
rebates and the number of requirements in a city's portfolio of conservation
policies}.
...
\added{We selected explanatory variables with the goal of understanding the
...
\replaced{We}{For the hydrological variables, we}
...
\deleted{We obtained populations for MSAs and states in 2010 and 2014
from the \textit{U.S. Census Bureau} [2015] and used them to calculate average
...
inflation and the regional cost of living [\textit{U.S. Bureau or Economic Analysis}, 2016].
We obtained RPI for MSAs and states in 2014
from the \textit{U.S. Bureau of Economic Analysis} [2016]. }%
...
\deleted{We calculated the Cook Partisan Voting Index (PVI)
using state- and county-level presidential votes, averaged over the 2008 and 2012
elections [\textit{Wasserman}, 2013, \textit{CQ Press}, 2016].
...
\added{In addition to the two political-economic variables and the two hydroclimatological variables,
...
\deleted{We tested for robustness by comparing this regression to
...
\added{PVI (Democratic Party preference), RPI (real per-capita income),}
the K\"oppen aridity index, surface water fraction,
\deleted{PVI, RPI, }metropolitan population, and population growth rate.
...
\added{We also considered a hierarchical varying-slope model, but with an average
...
Each city's \added{water-}conservation score (VWCI, requirements, or rebates) is
...
\deleted{$a_j$ }%
for city $i$ is adopted independently\added{,} with
\replaced{a}{the $m$\textsuperscript{th} action, $a_m$, having} probability
\replaced{$p_{ij}$}{$p_{im}$}.
In a pure binomial process, each policy in city $i$ would have the same
probability $p_i$ of being adopted.
\replaced{However, }{The variance of a pure binomial distribution is uniquely determined
by its mean and} the \added{variance of the} VWCI data was
\replaced{over-dispersed relative to}{greater than} a pure binomial
\added{distribution could account for,}
so our model uses a beta-binomial process, which allows for greater dispersion
of scores by drawing the probabilities \replaced{$p_{ij}$}{$p_{im}$}
...
\replaced{VWCI}{water-conservation policy measures}
\replaced{depends on}{are associated with} both
state-level and MSA-level covariates.
For city $i$, located in \replaced{$\text{state}_i$}{state $j$},
% \replaced{
% \begin{linenomath*}
% \begin{align}
...
and \replaced{$\alpha_{\text{state}_i}$}{$\alpha_j$} is the intercept of the MSA-level regression
for all cities in \replaced{$\text{state}_i$}{state $j$}.
...
In modeling \replaced{$\alpha_{\text{state}}$}{$\alpha_j$},
...
standard deviations of 0.5\added{, as recommended by \citet{gelman:prior:2008}
and \citet[pp.~55--57]{gelman:arm:2007}}\replaced{.}{:}
\added{
...
the state-average\replaced{.}{:}
\added{$x_{\text{scaled}} = (x - w) / (2 \sigma_w)$, where $x$ is an MSA-level
...
the mean over all 195~MSAs was zero and the standard deviation~0.5\replaced{.}{:}
\added{$x_{\text{scaled}} = (x - \mu_x) / (2 \sigma_x)$, as above.}

\added{We recognize that there are various ways that the multilevel model could be
configured.
...
\replaced{is given by}{of}
$\beta' N_{\text{action}}$ or $\gamma' N_{\text{action}}$.
\added{For example, $N_{\text{VWCI}} = 79$,
...
our data set \citep{gelman:bda:2014,vehtari:loo:2016}.
...
\deleted{We also considered a hierarchical varying-slope model, but with an average
...
\section{Results}
\replaced{Regressions}{With respect to the political-economic variables, regressions}
for VWCI, requirements, and rebates
(Figure~\ref{fig:vwci_cat_plot}; Tables~S15--S17)
all show\deleted{ important} correlations between
\replaced{PVI and urban water conservation}{adoption of water-conservation policies and Democratic-Party vote share}
at both the state and MSA levels.
\added{States with greater Democratic-leaning PVI scores have greater propensity to adopt conservation policies,
...
case \added{of }rebates, and it is negligible at the MSA level.}
\replaced{State-level}{With respect to the hydroclimatological variables, state-level}
aridity also correlates with all three conservation scores
\added{with MSAs in drier (low-aridity) states having a greater propensity for conservation.}
...
\added{Surface-water dependence does not correlate meaningfully with the conservation indices
...
\added{This finding was expected for reasons given above. }%
\deleted{RPI and surface-water dependence do not correlate meaningfully
with the conservation indices at the state level, except in the case of RPI and rebates,
...
{\centering \includegraphics[width=0.8\linewidth]{figures/vwci_cat_plot-1} 
...
\deleted{States with greater (Democratic-leaning) PVI have
greater propensity
...
variation in \replaced{RPI}{real personal income (RPI)} and MSA-level variation in aridity
do not correlate meaningfully with
total VWCI\replaced{ and}{. However, they do}
correlate differently with
requirements versus rebates:
state-level \replaced{RPI}{real personal income}
...
{\centering \includegraphics[width=0.8\linewidth]{figures/vwci_residuals-1} 
...
\replaced{We chose our explanatory variables based on theoretical considerations, as
described by \textit{Hess et al.} [2016]}{As described above, we chose our explanatory variables
...
\added{Both the regression coefficients and the information criteria were
indistinguishable for the different averaging periods.}

Second, we repeated the regression analysis
\added{with different sets of covariates:}
...
\added{We also repeated the regression analysis removing either PVI or aridity
(Figure~S7, Tables~S4 and~S7).
Removing either of these covariates did not significantly change
the regression coefficient of the remaining covariates, and the information
criteria scores were worse in both cases.
}

\added{In addition to comparing different choices of covariates, we also
...
The information criteria for the alternate normalization were slightly,
but not significantly, smaller than for the original normalization
(Tables~S9--S14), so the
...
We conclude from \replaced{this}{these robustness checks} that
\added{except for the regression coefficient for state-level PVI}
the results
...
\added{and model specifications}.
\added{Across all of these variations, the political variable measuring
...
\added{This finding suggests the importance of interdisciplinary thinking and
...
only, and rebates only), PVI is important at
\replaced{both the state and MSA levels}{the MSA and possibly also the state level}.
The correlation between
\replaced{PVI}{support for the Democratic Party}
...
\added{ and the results obtained
...
\replaced{RPI}{Real per-capita income (RPI)} measures the real purchasing power of
...
At the state level, greater \replaced{RPI}{income} correlates with lower conservation scores on
...
\deleted{higher (more Democratic-leaning) PVIs and }%
...
State-level \replaced{RPI}{income} does not have a clear correlation with VWCI;
...
\replaced{PVI than the state average}{preference for Democratic candidates}
and those
...
\added{Finally, dependence on surface water at either the state- or MSA-level
...
\replaced{of its low (Republican-leaning) PVI}{it leans Republican (low PVI)}.
...
\replaced{PVI}{partisan political preference (PVI)}
...
\replaced{PVI}{political preference},
and it could not quantitatively distinguish state-level from MSA-level effects of
\replaced{PVI}{political preference}
or other covariates.
Here we observe clearly that variations in\deleted{ both}
environmental\deleted{ and societal}
...
\added{We find that the role of state-level political preference is ambiguous and two formulations
...
\replaced{PVI, on the other hand, is consistently important at both the state and MSA
levels.}{Political preferences (how much a city or state leans toward the Democratic
