...
<<fit_models, echo=F, eval=resample_models, message=F, warning=F, error=F, results='hide', cache = do_cache && ! force_resampling, dependson='source_fit_model', cache.rebuild=!file.exists('data/model_fits.Rds'), cache.extra=c(random_alpha, std_data, vars_1, vars_2, file.mtime(file.path(script_dir, 'water_conserve_3_ml_vary_intercept_beta.stan')), file.mtime(file.path(script_dir, 'water_conserve_4_ml_vary_intercept_beta.stan')), file.mtime(file.path(script_dir, 'water_conserve_3_ml_vary_intercept.stan')))>>=
      process_models(std_data, vars = model_vars,
                   ...
                   seed = 2103112944)
@
<<test_models, echo=F, eval=resample_test_models, message=F, warning=F, error=F, results='hide', cache = do_cache && ! force_resampling, dependson='source_fit_model', cache.rebuild=!file.exists('data/model_fits.Rds'), cache.extra=c(random_alpha, std_data, vars_1, vars_2, file.mtime(file.path(script_dir, 'water_conserve_3_ml_vary_intercept_beta.stan')), file.mtime(file.path(script_dir, 'water_conserve_4_ml_vary_intercept_beta.stan')), file.mtime(file.path(script_dir, 'water_conserve_3_ml_vary_intercept.stan')))>>=
    process_models(std_data, vars = model_vars,
                   data.dir = 'data',
                   abs_rank = TRUE,
                   var_sel = c("vars_2", "vars_3", "vars_4", "vars_5",
                               "vars_6", "vars_7", "vars_8"),
...
States.
\added{This study does not attempt to establish causal relationships, but does
observe that}
\explain{Be clear that we're not claiming to demonstrate causality.}%
\replaced{Cities}{cities}
in states with arid climates and
\replaced{%
greater tendency to vote for Democratic candidates
}{%
higher partisan voting index (PVI)
}%
\explain{Tone down use of "Democratic" to address concerns about
...
\replaced{%
greater propensity than the rest of the state to vote Democratic
}{%
higher PVI than the rest of the state
}%
tend to adopt more conservation measures.
Economic factors and climatic differences between cities do not
\replaced{have much effect on}{correlate with}
\explain{Be clear that we're not claiming to demonstrate causality.}%
the number of measures adopted, but they
\replaced{impact}{do correspond to}
...
qualitative analyses on a subset of the \deleted{22} central cities of the \added{22} largest
metropolitan statistical areas (MSAs) in the extended Southwestern United States
\citep{hess:drought:2016}.
\added{That preliminary analysis found that propensity to
...
\explain{Edited for conciseness.}%

\deleted{
Our preliminary analysis of the 22~largest MSAs in the Southwest found that
propensity to adopt water conservation measures depended both on characteristics
of the physical environment (precipitation) and socio-economic and political
characteristics of the MSA (partisan voting and cost of living).
The Cook Partisan Voting Index for an MSA had, by a large margin,
the greatest predictive power for adopting water conservation measures.
This finding was supported by qualitative analysis, which suggested that
political association of water conservation with general ``environmentalist''
politics created political differences in the ways that cities respond to the
physical fact of water scarcity.
}
\explain{Break this paragraph up: Put a brief summary into the previous
paragraph, and incorporate details into the following discussion of
the PVI.}

\added{We represented the partisan political leanings of states and MSAs by the
...
\explain{Provide explanation of what PVI is.}

\added{Municipal water conservation policies are chosen by city officials, so it
...
\explain{Explain why why we use a measure (PVI) derived from presidential
vote shares, rather than something that uses vote shares from
...
\Sexpr{n_all_cities}~MSAs in \Sexpr{n_all_states} states \citep{hess:vwci:2017},
...
which variables
\replaced{best explain}{correlate most strongly with}
\explain{Be clear that we're not claiming to demonstrate causality.}%
...
%
<<vwci_histogram, echo=F, include=!for_submission, fig.height=6, fig.width=5, fig.pos='tb', fig.align="center", fig.cap=figure_captions['vwci_histogram'], cache=do_cache, dependson=c("load_model_fits","source_fit_models","captions"), cache.extra = c(theme_get()), results = fig_body_results>>=
std_data$msa_data %>%
...
Figure~\replaced{S1}{\ref{fig:vwci_map}})
\explain{Bring map figure from SI to main paper to help readers easily see the
...
\deleted{from }from~\Sexpr{min(std_data$msa_data$reqtotal)}
...
% top_vwci table
%
<<top_vwci, echo=F, include=!for_submission, dependson=c('load_model_fits','source_fit_models',"captions"), cache.extra=c(model_fits), results = tab_body_results>>=
...
\replaced{average annual average}{monthly average}
temperature ($T$) and \added{monthly} total precipitation ($P$) for the period
\replaced{1970}{1900}--2014
\explain{Present the time extent of the entire data set, rather than
just the years we extracted from that data set.}%
...
\added{We calculated the mean annual temperature and precipitation over a
30-year interval from 1985--2014.}
\explain{Explain interval used to calculate aridity averages.}%
...
We obtained populations for MSAs and states in 2010 and \Sexpr{pop_target_year}
from the \citet{census:population:2015} and used them to calculate average
annual rates of population growth.
\added{An important economic indicator is real personal income (RPI),
...
\explain{Explain what the RPI variable indicates and why we used it.}%
\replaced{%
Per-capita real personal income (RPI) for MSAs and states in \Sexpr{bea_year}
were was obtained
}{%
We obtained RPI for MSAs and states in \Sexpr{bea_year}
}%
from the \citet{bea:rpi:2016}.
\replaced{The}{We obtained the}
...
\replaced{%
The Cook Partisan Voting Index (PVI) was calculated
}{%
...
(Tables~S1--S2 and Datasets~S1--\replaced{S2}{S4}).
...
\replaced{deemed an especially reliable index of aridity}%
...
\explain{Give a clearer description of what the Quan paper said and why we chose
the K\"oppen index, as opposed to a more detailed one that uses
evapotranspiration.}%
...
a few large cities producing an asymmetric fat tail (Figure~S\replaced{2}{1}),
so we deemed \added{the} natural logarithm of population
\replaced{more suitable}{a more suitable predictor}
for a regression analysis \citep[pp.~59--61]{gelman:arm:2007}.

\added{We tested for robustness by comparing this regression to
alternate regression analyses that used different predictors and different
prior distributions.}
\explain{Address questions about possible alternate covariates, such as
economic inequality, population density, MSA area, etc.}

\subsection{Regression Analysis}
We applied Bayesian hierarchical varying-intercept
\added{logistic} regression to each of the
...
The regression
\replaced{%
models each city's conservation score (VWCI, requirements, or rebates)
as the result of a beta-binomial process (an overdispersed analogue to
a binomial process) \citep[pp.~437--38]{gelman:bda:2014}.
This model characterizes each city $i$ by a probability $p_i$ for adopting water
conservation policies. In a pure binomial process, each policy would have the
same probability $p_i$ of being adopted in city $i$, but the beta-binomial
process allows for greater dispersion of scores by drawing the probability for
each action separately from a beta distribution with mean $p_i$.
The probability $p_i$ is modeled using a hierarchical varying-intercept logistic
regression on both state-level and MSA-level covariates:
}{%
...
\explain{Added extensive description of the regression analysis, and be explicit
in our references to standard textbook treatments.}
...
\explain{Clearer description of the regression parameters.}%
\begin{linenomath*}
\begin{align}
...
\explain{Detailed descriptions of the regression parameters and explain
why we did not explicitly model noise in the probabilities $p_i$.}

\added{In modeling $\alpha_{\text{state}}$,
$\alpha_0$ is the mean intercept across all states,}
\explain{Clarify $\alpha_0$ parameter.}%
...
\deleted{,
and $\phi$ parameterizes the overdispersion, relative to a binomial distribution.
As $\phi \rightarrow \infty$, the distribution approaches a binomial distribution
}%
.
...
\explain{Explain clearly the point of standardizing independent variables.}%
...
MSA level,
\replaced{the MSA-level covariate was rescaled}
{we addressed multicollinearity between state-level and MSA-level
variables by rescaling the MSA-level covariate}
\explain{Clarify the rescaling procedure for independent variables.}%
to the same scale as its state-level counterpart and
\replaced{the state-level value was subtracted}{subtracting the state-level value},
...
\deleted{Standardizing the independent variables served both to facilitate
comparison of the relative importance of variables whose natural scales are
vastly different \citep{gelman:prior:2008,gelman:arm:2007}, and also to improve
computational performance \citep{stan:manual:2015}.}

\added{Because there are no previous quantitative comprehensive analyses of urban
...
\explain{Explain our choice of priors.}%
\replaced{%
We represent $\delta$ as normally distributed
and
use weakly informative Cauchy priors for the coefficients and parameters
\citep{gelman:prior:2008}:
}{%
...
\explain{Explain that our choice of Cauchy priors follows established practice
and be clearer about the source for this choice.}%
\begin{linenomath*}
\begin{align}
...
\end{linenomath*}
\deleted{%
The posterior distributions were considerably narrower than the priors, and were
not sensitive to changing the scales of the priors.
}
...
We
\replaced{%
ran each chain for 2000 iterations (1000 for warm-up and 1000 for sampling),
}{%
sampled four Markov chains for 1000 iterations each, after 1000 warm-up iterations,}
\explain{Clearer explanation of the Markov-Chain Monte Carlo sampling.}%
...
\explain{Clarify where we get the rescaled regression coefficients from.}%
...
\deleted{ \citep[pp.~81--82]{gelman:arm:2007}}.
...
using leave-\replaced{on}{one}-out cross-validation (LOO)
(\replaced{Table~S3}{Tables~S3--S4}),
the Widely Applicable Information Criterion (WAIC)
(\replaced{Table~S4}{Tables~S5--S6}),
and
\replaced{graphical}{visual}
...
\replaced{hierarchial}{hierarchical}
...
of only \Sexpr{round(n_cities / n_states, 1)} MSAs per state there were too
few degrees of freedom to adequately constrain \Sexpr{length(msa_vars_2)}
additional regression coefficients for each state.
}%
\explain{Explain why we did not consider a varying-slope model.}

\section{Results}
\deleted{%
In comparing magnitudes of regression coefficients $\beta$ and $\gamma$
(see Section~\ref{sec:data.methods}), we refer to the average value of the
posterior (means and medians were equal within $\pm 0.01$; see Tables~S5--S7).
Where the 95\% highest-density interval of the posterior overlaps zero, we refer
to the effect as ambiguous because it is consistent with zero and its sign cannot
be determined with confidence.
}%
\explain{We moved paragraph that originally appeared here to the previous section.}

Regressions for VWCI, requirements, and rebates
(\replaced{%
Figures~2--3; Tables~S5--S7%
}{%
Figure~\ref{fig:vwci_cat_plot}; Tables~S7--S9%
})
all show
\replaced{%
that PVI is an important predictor of
}{%
...
\replaced{%
Aridity matters at the state level for
}{%
...
\replaced{effect on}{correlation with}
requirements, but
\replaced{make only small and ambiguous contributions to}{not with}
VWCI and rebates.
At the MSA level, faster population growth and larger population
\replaced{are associated}{correlate}
with higher \replaced{scores on}{values for} all three conservation scores.
\deleted{Effects of}
RPI and surface-water dependence
\replaced{are mostly ambiguous}{do not correlate meaningfully with the
conservation indices}
at the state level,
\deleted{with uncertainty making it difficult to say anything definite}
except in the case of RPI and rebates,
and are negligible at the MSA level.
%
% vwci_cat_plot
%
<<vwci_cat_plot, echo=F, include=!for_submission, fig.height=7.75, fig.width=5, fig.pos='p', fig.align="center", fig.cap=figure_captions['vwci_cat_plot'], cache=do_cache, dependson=c("load_model_fits","source_fit_models","captions"), cache.extra = c(theme_get(), thick_ci, thin_ci), results = fig_body_results>>=

limits <- model_fits$ggs$ggs_2_ml_beta_alpha %>% dplyr::filter(str_detect(Parameter, '^(beta|gamma)')) %>%
...
levels <- levels(model_fits$ggs$ggs_2_ml_beta_alpha$Parameter)

p_vwci <- generate_cat_plot(model_fits$ggs$ggs_2_ml_beta_alpha, "vwci", " (a)VWCI",
                        legend = FALSE, levels = levels) +
  theme(axis.title.x = element_blank())
p_req <- generate_cat_plot(model_fits$ggs$ggs_2_ml_beta_alpha_req, "reqtotal", "(b) Requirements",
                        limits, legend = TRUE, levels = levels) +
  theme(axis.title.x = element_blank(), legend.position = c(0.99,0.01))
p_reb <- generate_cat_plot(model_fits$ggs$ggs_2_ml_beta_alpha_reb, "rebtotal", "(c) Rebates",
...
States with \replaced{more positive}{greater} (Democratic-leaning) PVI have
greater propensity
to adopt conservation policies, as do MSAs whose PVI is greater
(more likely to vote Democratic) than the rest of the state.
States with drier climates (\replaced{negative}{lower} aridity) also have
...
\replaced{each has ambiguous effects on}{do not correlate meaningfully with}
\explain{Be clear that we're not claiming to demonstrate causality.}%
total VWCI and
\replaced{different effects on}{correlate differently with}
requirements versus rebates:
state-level RPI
\replaced{affects rebates more strongly}{correlates more strongly with rebates}
than \added{with} requirements
and MSA-level variations in aridity
\replaced{affect requirements more strongly}{correlate more strongly with requirements}
than \added{with} rebates.
...
among the coefficients (\replaced{Figures.~S3--S5}{Figures.~S2--S4}).
%
% vwci_residuals
%
<<vwci_residuals, echo=F, include=!for_submission, fig.height=5, fig.width=6, fig.pos='tb', fig.align="center", fig.dev="pdf", fig.cap=figure_captions['vwci_residuals'], cache = do_cache, dependson=c('load_model_fits','source_fit_models',"captions"), cache.extra=c(theme_get(),model_fits), results = fig_body_results>>=
if (TRUE) {
  ...
  # p4 <- make_scatter_plot(res, color = TRUE, pt.alpha = 0.4, residual.plot = TRUE)
  ...
  p4 <- model_fits$res$res_2_ml_beta_alpha$residuals %>% ggplot(aes(x = predicted, y = residual)) +
...
% vwci_top_residuals
%
<<vwci_top_residuals, echo=F, include=!for_submission, results='asis', cache = do_cache, dependson=c('load_model_fits','source_fit_models','captions'), cache.extra=c(model_fits), results = tab_body_results>>=
tab_placement = ifelse(for_submission, "htbp", "tbp")
table_data <- model_fits$res$res_2_ml_beta_alpha$residuals %>% arrange(desc(abs(residual))) %>%
  mutate(city = str_c(city, as.character(state), sep = ', ')) %>%
...
\added{We chose our explanatory variables based on theoretical considerations, as
...
\explain{Add robustness checks and discussion of possible alternate predictor
variables, such as population density, MSA area, and economic inequality,
different averaging periods for calculating MSA aridity,
and different priors.}

\added{First, we tested whether conservation policies might be more
...
\added{Finally, we tested for sensitivity to the functional form of the prior
...
\added{We conclude from this that the results of our analysis are robust against
many changes of time-spans, explanatory variables, and assumptions about priors.}

\added{There are myriad other potential explanatory variables,
...
\citep{hess:drought:2016}.}
\explain{Explain why we did not extend this analysis to consider a more
comprehensive set of alternate explanatory variables}
...
conservation is \replaced{driven}{associated} both \replaced{by}{with}
characteristics of the physical environment and
\replaced{by}{with} political and demographic
%, and economic
...
The
\replaced{effect of}{correlation between}
\explain{Be clear that we're not claiming to demonstrate causality.}%
PVI
\replaced{on all}{and the}
...
\replaced{effect}{correlations}
\explain{Be clear that we're not claiming to demonstrate causality.}%
that are consistent across all three conservation scores.
The
\replaced{effects of}{correlations between}
MSA-level aridity
\replaced{within a state on}{and}
VWCI and rebates are much
smaller\deleted{, ambiguous,}
...
\replaced{effect}{correlation with VWCI and requirements}
\explain{Be clear that we're not claiming to demonstrate causality.}%
is small and
\replaced{ambiguous (consistent with zero)}{consistent with zero},
...
At the MSA-level,
\replaced{variations of RPI within a state}{correlations of all three scores with RPI}
\replaced{have negligibly small effects, which are}{are negligibly small and}
consistent with zero\deleted{, on all three scores}.
...
\explain{Be clear that we're not claiming to demonstrate causality.}%
VWCI\replaced{,
but RPI appears to affect the composition of conservation policies, with lower
RPI corresponding to a greater reliance on rebates.
}{;
...
total VWCI, but
\replaced{%
they do affect the composition of policies, with MSAs whose climate is more arid
than the state average tending%
}{%
more arid MSAs tend%
}
...
\replaced{affect}{correlate with}
the number of conservation policies adopted, but
\replaced{affects}{with}
...
\replaced{%
This analysis finds that for the most part hydroclimatological variables
are important at the state level, but not at the MSA level, whereas PVI is
important at both the state and MSA level.
}{%
...
variable \replaced{that}{whose effect} is consistently clearly distinct
...
\replaced{appears to affect which}{does correlate with the}
...
\replaced{will be}{are}
more likely to
adopt water conservation policies.

\replaced{Policy}{We conclude that policy}
...
\added{We also expect that further integrated interdisciplinary research along
...
\explain{Be clear that we do not aspire to be the last word, but that there is
a lot of potential for future research to consider other interdisciplinary
questions about water conservation policies.}
...
All data used for this paper are \deleted{properly} cited and listed in the references or
included in the supplementary information.
\replaced{The complete VWCI data set, together with the code (R scripts and Stan models) used for this analysis, will also be posted publicly on github prior to publication.}{The complete VWCI data set with
...
\explain{URL and DOI for data used in this paper, which we have posted to figshare.}%
%
% Bibliography
