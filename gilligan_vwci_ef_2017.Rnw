...
\replaced{\Sexpr{n_all_states} states}{the United States} \citep{hess:vwci:2017},
which\added{ span \Sexpr{n_all_states} and} comprise more than half of the 382~MSAs
...
\added{The set of \Sexpr{n_actions} measures for each city makes it possible to construct different variables.
...
\deleted{We obtained populations for MSAs and states in 2010 and \Sexpr{pop_target_year}
...
the mean over all \Sexpr{n_cities}~MSAs was zero and the standard deviation~0.5\replaced{.}{:}
...
\added{For example, $N_{\text{VWCI}} = \Sexpr{n_actions}$,
...
few degrees of freedom to adequately constrain \Sexpr{length(msa_vars_1)}
