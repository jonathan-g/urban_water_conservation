...
% vwci_cat_plot
%
<<vwci_cat_plot, echo=F, include=!for_submission, fig.height=7.75, fig.width=5, fig.pos='p', fig.align="center", fig.cap=figure_captions['vwci_cat_plot'], cache=do_cache, cache.extra = c(theme_get(), thick_ci, thin_ci), results = fig_body_results>>=
...
% vwci_residual_range
%
<<vwci_residual_range, echo=F, include=F, results='hide', cache=do_cache, cache.extra=c(model_fits)>>=
...
% vwci_residuals
%
<<vwci_residuals, echo=F, include=!for_submission, fig.height=5, fig.width=6, fig.pos='tb', fig.align="center", fig.dev="pdf", fig.cap=figure_captions['vwci_residuals'], cache = do_cache, cache.extra=c(theme_get(),model_fits), results = fig_body_results>>=
if (TRUE) {
  ...
  p_data = p4$data %>% dplyr::filter(! is.na(res_label))
...
% vwci_top_residuals
%
<<vwci_top_residuals, echo=F, include=!for_submission, results='asis', cache = do_cache, cache.extra=c(model_fits), results = tab_body_results>>=
tab_placement = ifelse(for_submission, "htbp", "tbp")
...
covariates is available from \citet{gilligan:vwci.data:2018}.
